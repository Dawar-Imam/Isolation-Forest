...
def distribution_of_transactions(data):
    ...
    plt.savefig('Distribution of Transaction Amount.png')
    plt.show()
...
def visualize_anomalies(data):
    ...
    plt.savefig('Anomalies in Transaction Amount.png')
    plt.show()
...
def evaluate_model(data, model):
    """
    Evaluates the performance of the model on the given data and returns a classification report and accuracy.

    Args:
        data (array-like): The input data to be evaluated.
        model: The trained isolation forest model.

    Returns:
        tuple: A tuple containing the classification report (dictionary) and accuracy (float).
    """
    predictions = model.predict(data)
    predictions = np.where(predictions == 1, 1, 0)
    true_labels = np.ones(len(predictions))
    target_names = ['Anomaly', 'Normal']
    report = classification_report(true_labels, predictions, target_names=target_names, digits=4, output_dict=True)
    accuracy = report['accuracy']
    report['support'] = {'Anomaly': np.sum(predictions == 0), 'Normal': np.sum(predictions == 1)}
    
    return report, accuracy

def print_report(report):
    """
    Prints a formatted matrix containing precision, recall, f1-score, and support for different classes.

    Args:
        report (dict): The classification report generated by evaluate_model function.
    """
    metrics = ['precision', 'recall', 'f1-score', 'support']
    classes = ['Anomaly', 'Normal', 'accuracy', 'macro avg', 'weighted avg']
    matrix_data = {}
    for cls in classes:
        matrix_data[cls] = [report[cls][metric] if isinstance(report[cls], dict) else report[cls] for metric in metrics]
    matrix_df = pd.DataFrame(matrix_data, index=metrics)
    matrix_df = matrix_df.transpose()
    print(matrix_df)


report, accuracy = evaluate_model(relevant_data, model)
